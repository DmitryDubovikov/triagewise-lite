...
красные. «Хуже» здесь значит именно сломанный формат: правильность меток эти ворота осознанно не
судят (разбор ниже). Оба пути к красному проверены живьём.
...
**DVC** здесь не новый инструмент (перенос из sentiment-mlops), но напомнить, как он устроен, стоит.
Это «git для данных». Файл данных отдаётся под управление DVC (`dvc add data/golden.jsonl`): DVC
считает его md5-хэш и кладёт рядом маленький текстовый указатель `data/golden.jsonl.dvc`. В git
коммитится только указатель; сам файл командой `dvc push` уезжает в отдельное хранилище — у нас это
локальная папка `../triagewise-lite-dvc-remote`, соседка репо. На чистом клоне файл восстанавливается
командой `dvc pull`: DVC читает хэш из указателя и достаёт из хранилища ровно ту версию, на которую
указывает текущий коммит.

Почему DVC, а не просто закоммитить golden в git? Честно: 40 тикетов git бы пережил — дело не в
размере, а в трёх вещах. Во-первых, golden-сет — это данные со своим жизненным циклом, а не код: он
будет расти числом тикетов и меняться пачками (в iter 5 приедет вторая пачка «после релиза продукта»
для моделирования дрейфа), и каждая такая пачка — это новая *версия датасета*, а не правка кода;
git-диффы по JSONL с переразметкой нечитаемы и только замусоривают историю кода. Во-вторых, md5 из
`.dvc`-указателя здесь — рабочий механизм, а не формальность: он продублирован в шапках сгенерённых
eval-ассетов, и sync-тест по нему ловит рассинхрон «ассеты собраны из другой версии golden». В git
«версия файла данных» — это неявный побочный продукт коммита; в DVC — первоклассный факт с хэшем, на
который можно ссылаться и который можно проверять. В-третьих, это перенос дисциплины из
sentiment-mlops один-в-один: тот же паттерн «версия данных отделена от версии кода», что и для
train-сетов, — здесь применён к golden-сету, и north-star-строка резюме говорит именно «versioned
golden set». Побочный эффект решения — CI не умеет `dvc pull` (remote локальный), поэтому ворота в
CI работают по производным ассетам, а не по golden напрямую; это разобрано в build-шаге ниже.
...
**Что именно значит «промпт стал хуже».** У ворот один ассерт — `is-json`, и его строгость
несимметрична; это стоит проговорить явно, потому что слово «хуже» обещает больше, чем ворота
проверяют. Строгая половина: ответ модели **целиком** должен парситься как JSON — promptfoo делает
буквально `JSON.parse` всего ответа, поэтому «Вот результат: {…}» или JSON в markdown-фенсах красные
сразу (именно от этого champion-промпт заканчивается «No prose, no code fences»). Дальше объект
сверяется со схемой: все пять полей обязательны, `priority` и `sentiment` — жёсткие enum'ы
(`"critical"` — красный), `needs_human` — строго boolean (строка `"yes"` — красный). Нестрогая
половина: `category` и `draft_reply` — любая строка (у `category` нет enum'а, `draft_reply` пройдёт
даже пустым), лишние поля схема тоже пропускает. И главный edge case: разметка golden — правильная
категория, приоритет и т.д. — лежит в `eval/tests.yaml` как `metadata.expected`, для человека, и ни
один ассерт по ней не судит. Промпт, который стал ставить `priority: low` тикету «SSO упал у всей
команды перед релизом», проходит ворота зелёным. Это осознанно: accuracy — не ворота (правило 1),
семантическую деградацию должны ловить champion/challenger-сравнение (iter 6) и мониторинг дрейфа
(iter 5). Отсутствие enum'а на `category` — тоже дизайн, а не забывчивость: дрейф в iter 5
моделируется появлением новой категории тикетов, и жёсткий список делал бы ворота ложно-красными на
легитимном дрейфе. Итого assert-red — это ровно четыре сценария: проза или фенсы вокруг JSON,
невалидный JSON, пропавшее обязательное поле, значение вне enum'а (или не-boolean в `needs_human`).
...
Чего в этой итерации **НЕ** происходит (типичная путаница). Правильность триажа не проверяется —
ассерты судят только формат (подробный разбор выше); label-гейт и сравнение champion-vs-challenger —
территория iter 6. promptfoo в CI **не звонит модели вообще** — ни ключа, ни
