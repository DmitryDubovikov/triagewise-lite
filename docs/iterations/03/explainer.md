...
Нового инструмента в проекте нет (LiteLLM — перенос из policywise), но раз итерация делает его
героем, представим его с нуля — плюс два понятия, которыми оперирует остальной документ.

**LiteLLM** — это Python-библиотека-переходник к LLM-провайдерам: она даёт один и тот же интерфейс
вызова (`completion`/`acompletion` в формате OpenAI chat-API) поверх сотни провайдеров — OpenAI,
Anthropic, локальные серверы и т.д. Код зовёт `litellm.acompletion(model="...", messages=[...])`,
а LiteLLM сам разбирается, к какому API и как сходить. Употреблять его можно двумя способами:
как **SDK** — обычная библиотека внутри твоего процесса (наш путь), либо как **Proxy** — отдельный
сервер-гейтвей, через который ходят все клиенты. Proxy у нас запрещён конституцией (правило 5):
это лишняя сетевая поверхность с реальным следом CVE, а нам нужен ровно один контролируемый
исходящий вызов.

В проект LiteLLM внедрён максимально узко, и эта узость — осознанная дисциплина. Единственное
место, где SDK вообще упоминается, — функция `_live_completion()` внутри `app/llm/router.py`:
все слои выше зовут `route("tier", ...)` и про LiteLLM не знают. Импорт ленивый — в режиме
`replay` (дефолт) SDK не загружается в процесс вовсе; телеметрия выключена и все callback-каналы
обнулены до первого вызова; версия запиннена (`litellm>=1.55,<2` в `pyproject.toml`, точная
1.90.0 — в `uv.lock`). С этой итерации всё перечисленное проверяет pytest, а не ревьюер по памяти.
Зачем вообще прослойка, если провайдер у нас один OpenAI? Во-первых, смена провайдера остаётся
правкой одной строки в `llm-tiers.yaml`, а не переписыванием вызова. Во-вторых — и для этой
итерации главное — LiteLLM возит с собой утилиты вокруг вызова, в частности таблицу прайсингов
моделей, на которой построен следующий абзац.

**`litellm.completion_cost`** — функция из того же SDK, которая по ответу модели считает цену вызова
