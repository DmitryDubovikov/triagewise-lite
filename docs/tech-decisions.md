...
- **Prefect server** (iter 6b) — deployments + интервальный scheduler continuous-evaluation петли; stateful (sqlite в volume), симметрично MLflow/Phoenix. Исходный план «lib, in-process serve» не пережил встречи с Prefect 3: расписание там исполняется **строго сервером** (ephemeral-режим осознанно поставляется без scheduler-сервиса) — развилку см. ниже. **Flow-runner — на хосте** (`make loop`): контейнер только хранит стейт и тикает расписание, триаж не исполняет и в OpenAI не ходит.

**На хосте:**
- **Приложение** (`app/` CLI/UI) + **все Python-deps в одном uv-env**: LiteLLM SDK (lib in-process; Proxy запрещён правилом 5 → сервиса нет), **Prefect** (iter 6b — lib: `@flow` + serve()-runner; сам server — Compose-сервис, см. выше), **DVC**, ruff, pytest. Это deps, не рантаймы → не клаттер; Docker тут добавил бы вес ради ничего.
...
- **Prefect server — Compose-сервис, а не «in-process serve» (правка решения, iter 6b).** Исходная ставка была «Prefect строго lib, flow.serve() in-process, сервера нет». Реальность Prefect 3: `serve()` с интервалом лишь регистрирует расписание, а исполняет его **scheduler-сервис внутри сервера** — в ephemeral-режиме он отключён намеренно (`run_in_ephemeral=False`), то есть тики не наступают никогда. Развилка решалась между transient-сервером внутри `make loop` (shell-обвязка, ~10s буста на запуск), driver-циклом `while+sleep` (расписание наше, а не Prefect — слабее для резюме) и Compose-сервисом. Взяли **Compose**: Prefect server — ровно тот stateful control-plane бэкенд, который по принципу границы исполнения и так живёт в Docker (симметрия MLflow/Phoenix), а «родное» расписание Prefect — честная демонстрация continuous-evaluation. Flow-runner остаётся хост-процессом — LLM-egress границу не трогаем.
...
- **Реестр-хендл (MLflow client) открывается на транспортном boundary** и передаётся вниз аргументом. Запись версий/aliases промптов — только через persistence-слой: seed — boundary-скриптом (`register_prompt`), alias `champion` двигает промоушен-флоу (iter 6a) вызовом persistence-функции; domain реестра не видит.
